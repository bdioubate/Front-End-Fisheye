class contact{
    ...
    validateChamp() {
        ...
        for(let i = 0;i < champ.length;i++) {
          ...
          if((champ.id === "prenom") && champ.value.length < 2 ) {
            throw new Error(`Votre prénom doit comporter au moins 2 caractères ! ${champ.id}`)
          }
          else if((champ.id === "nom") && champ.value.length < 2  ) {
            throw new Error(`Votre nom doit comporter au moins 2 caractères ! ${champ.id}`)
          }
          else if(champ.id === "email" && regex.test(champ.value) === false) {
            throw new Error(`Veuillez remplir correctement votre e-mail ! ${champ.id}`)
          }
          else if(champ.id === "message" && champ.value.length < 5) {
            throw new Error(`Votre message doit comporter au moins 5 caractères ! ${champ.id}`)
          }
          else {
            this.SupprimeMessageErreur(`${champ.id}`)
          }*/
      }
